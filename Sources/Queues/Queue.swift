...
extension Queue {
    ...
    public func dispatch<J: Job>(
        ...
        return self.set(id, to: storage).flatMap {
            ...
            //self.updateMetrics(job)
            self.logger.info("Dispatched queue job")
            return self.sendNotification(of: "dispatch", logger: logger) {
                $0.dispatched(
                    job: .init(id: id.string, queueName: self.queueName.string, jobData: storage),
                    eventLoop: eventLoop
                )
            }
        }
    ...
    //    func updateMetrics<J: Job>(_ job: J.Type) {
        ...
        return self.configuration.notificationHooks.map {
            notification($0).flatMapErrorWithEventLoop { error, eventLoop in
                logger.warning(
                    ...
                    metadata: ["kind": "\(kind)", "error": "\(String(reflecting: error))"]
                )
