...
public struct QueueWorker: Sendable {
    ...
    private func runOneJob() async throws -> Bool {
        ...
        guard (data.delayUntil ?? .distantPast) < Date() else {
            logger.trace(
                "Job is delayed, requeueing for later execution",
                metadata: ["delayed-until": "\(data.delayUntil ?? .distantPast)"]
            )
        ...
        //self.updateMetrics_JobsInProgress()

        try await self.runOneJob(id: id, job: job, jobData: data, logger: logger)
        return true
    }

    //    private func updateMetrics_JobsInProgress() {
    //        Meter(
    ...
    private func runOneJob(id: JobIdentifier, job: any AnyJob, jobData: JobData, logger: Logger) async throws {
        ...
        logger.info(
            "Dequeing and running job",
            metadata: ["attempt": "\(jobData.currentAttempt)", "retries-left": "\(jobData.remainingAttempts)"]
        )
        do {
            ...
            if jobData.remainingAttempts > 0 {
                ...
                logger.warning(
                    "Job failed, no retries remaining",
                    metadata: ["error": "\(String(reflecting: error))", "attempts-made": "\(jobData.currentAttempt)"]
                )
    ...
    private func retry(
        ...
        logger.warning(
            "Job failed, retrying",
            metadata: [
                "retry-delay": "\(delay)", "error": "\(String(reflecting: error))",
                "next-attempt": "\(updatedData.currentAttempt)", "retries-left": "\(updatedData.remainingAttempts)",
            ]
        )
        try await self.queue.clear(id).get()
