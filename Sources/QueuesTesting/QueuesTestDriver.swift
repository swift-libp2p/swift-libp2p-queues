...
extension Application.Queues {
    public final class TestQueueStorage: Sendable {
        ...
        public func first<J: Job>(_: J.Type) -> J.Payload? {
            ...
            guard let queueJob = self.queue.first(where: { filteredJobIds.contains($0) }),
                let jobData = self.jobs[queueJob]
            else {
                return nil
            }
...
struct AsyncTestQueue: AsyncQueue {
    ...
    func get(_ id: JobIdentifier) async throws -> JobData {
        self._context.withLockedValue { $0.application.queues.asyncTest.jobs[id]! }
    }
    func set(_ id: JobIdentifier, to data: JobData) async throws {
        self._context.withLockedValue { $0.application.queues.asyncTest.jobs[id] = data }
    }
    func clear(_ id: JobIdentifier) async throws {
        self._context.withLockedValue { $0.application.queues.asyncTest.jobs[id] = nil }
    }
    func pop() async throws -> JobIdentifier? {
        self._context.withLockedValue { $0.application.queues.asyncTest.queue.popLast() }
    }
    func push(_ id: JobIdentifier) async throws {
        self._context.withLockedValue { $0.application.queues.asyncTest.queue.append(id) }
    }
}
